...
async def update_tomorrow(dp: Dispatcher, message: Message):
    ...
    if not mw.auth_status:
        ...
        try:
            ...
            await dp.bot.send_message(text='Ошибка получения списка папок на почтовом сервере',
                                      chat_id=message.chat.id,
                                      reply_markup=None)
    ...
    if not result:
        ...
        try:
            ...
            await dp.bot.send_message(text='Нет новых писем с меню',
                                      chat_id=message.chat.id,
                                      reply_markup=None)

        return
    for msg in mw.messages:
        with open(os.path.join(mw.save_dir, str(msg.subject.uk) + '.pdf'), "wb") as fp:
            fp.write(msg.file.content)
            fp.close()
    ...
    if not sw.authorized:
        ...
        try:
            ...
            await dp.bot.send_message(text='Ошибка авторизации на школьном сайте',
                                      chat_id=message.chat.id,
                                      reply_markup=None)
    ...
    sw.upload_files(folder_path=dp.bot.get('config').misc.files_paths.tomorrow_folder_path,
                    files=[os.path.join(dp.bot.get('config').misc.files_paths.local_dir_compressed_path, file) for file
                           in
    ...
    try:
        ...
        await dp.bot.send_message(text='Файлы с меню на завтра размещены',
                                  chat_id=message.chat.id,
                                  reply_markup=None)
